...
This project is a full‑size (180 cm, ~50 kg) walking robot with onboard vision‑language AI, with a target BOM under $3,500.  
...
- A **stripped gaming laptop (RTX 2070)** as the main AI brain (an affordable alternative to expensive specialised modules, offering better performance for the price).
- A **distributed control network** – ESP32‑S3 as CAN gateway, GD32F303 nodes for low‑level FOC, and hacked hoverboard ESCs for medium motors.
...
| Parameter              | Value                                                         |
| :--------------------- | :------------------------------------------------------------ |
| Height / Target Weight | 180 cm / 45–50 kg                                             |
| Degrees of Freedom     | 36 (6 High‑torque, 17 Medium, 13 Base)                        |
| Battery                | 8S LiFePO₄ (Liitokala 50Ah) + JK Smart BMS (200A peak)        |
| Main Power Bus         | 28.8 V (nominal 25.6 V)                                       |
| Secondary Rails        | 19.5 V (laptop), 5 V (logic & display), 6 V / 12 V (future)   |
| Compute                | Laptop RTX 2070 8GB + 16GB DDR4 (Ubuntu 22.04 + RT‑Preempt)   |
...
| Communication          | CAN (TJA1050) + SPI (encoders) + UART (ESC bridge)            |
| Sensors                | 11× BMI160+QMC5883L, 10× FSR402, 4× XNQJALYCY 50kg load cells |
...
- **Battery:** 8S LiFePO₄ (60145 cells, 50Ah) – chosen for safety and high discharge rate.  
- **BMS:** JK‑B2A8S20P (200A continuous) with Bluetooth monitoring.  
- **Main protection:** 200A DC circuit breaker on the positive line.  
- **Fuse hub:** 10‑slot ATO fuse block – 40A for motor groups, 25A for laptop and 5V converters, 10A for head peripherals.  
- **Bus capacitors:** 6× 4700 µF / 50V aluminium electrolytic on the main rail (absorbs regenerative spikes).  
...
3. **Low‑level (joint controllers)**  
   ...
   – **Hoverboard ESCs** (x9) – each drives 2 medium motors (5065). Total 18 channels, but only 17 are used (one spare). They only have **UART**, so we use **GD32F303 microcontrollers** as UART‑to‑CAN bridges: one GD32 polls 2–3 ESCs and relays data to ESP32 via CAN.  
   – **SimpleFOCMini** (x13) – drives the 13 base motors (3205) directly from GD32F303 boards (which also handle the SPI encoders).  
...
- **Foot force:** 4× XNQJALYCY 50 kg load cells (2 per foot) → HX711 ADC → ESP32.

---

## Project Status (March 2026)

- [x] Full component selection and cost estimation (see `/docs/BOM_Cost_Analysis.md`).
...
- [ ] Firmware for ESP32 and GD32 (will be published once tested).
...
* [`docs/power_schematic.en.md`](docs/power_schematic.en.md) – Power distribution diagram, fuse allocation (200A breaker, ATO hub), and WAGO terminal block wiring.
...
* [`software/gd32_firmware/`](./software/gd32_firmware/) – GD32F303 firmware: SimpleFOCMini driver, MT6826S encoder reading (SPI), and ESC UART communication.
