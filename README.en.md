...
The goal is to build a full‑size (180 cm, ~60 kg) walking robot with onboard AI (vision + language models) for a budget of about **350,000 RUB**. Instead of expensive industrial actuators, we use:

- **DIY two‑stage strain‑wave reducers with intermediate rolling elements (ПТК)** – standard bearing rollers as the rolling elements.
...
- **Distributed control network:** ESP32‑S3 as a CAN gateway, AT32F403ACGU7 for low‑level FOC, and repurposed hoverboard mainboards for medium and basic motors.

All mechanical parts are designed for **home‑cast aluminium** (using a charcoal furnace) or **3D printing** (PETG / PETG‑CF) on an Ender 3 V3 Plus.
...
| Parameter                | Value                                                                        |
| :----------------------- | :--------------------------------------------------------------------------- |
| Height / Weight          | 180 cm / ~60 kg (target)                                                     |
| Degrees of Freedom       | 36 (6 high‑power + 7 medium + 10 basic + 13 low‑power)                       |
| Battery                  | 10S Li‑NMC (SK 3.7V 80.5Ah) + JK Smart BMS (200A)                            |
| Main power bus           | 33–41 V (cell voltage limited to 3.3–4.1 V)                                  |
| Secondary voltages       | 24 V (SimpleFOCmini), 20 V (laptop), 5 V (logic + display)                   |
...
| Communication            | CAN (STM32F103C8T6+SN65HVD230)                                               |
| Sensors                  | 13× BMI160, 1× ICM‑20948, 10× FSR402, 4× XNQJALYCY 100 kg load cells         |
| Budget (August 2026)     | ~329,043 RUB (full breakdown in [BOM.en.md](./docs/BOM.en.md))               |
...
- **Legs (8 actuators):**  
  ...
  – **2× 5010 360KV** (reduction 1:64) – hip rotation (one per leg).

- **Torso and neck (4 actuators):**  
  – **1× 5065 140KV** (1:64) – torso tilt (waist).  
  – **3× 3205 110KV** (1:42) – neck (2 tilts + 1 pan).

- **Arms (24 actuators):**  
  – **4× 5065 140KV** (1:64) – shoulders (2 per arm: elevation and abduction).  
  – **2× 5010 360KV** (1:42) – shoulders (rotation, 1 per arm).  
  – **2× 5065 140KV** (1:64) – elbows (flexion, 1 per arm).  
  – **2× 5010 360KV** (1:42) – elbows (forearm rotation, 1 per arm).  
  – **4× 5010 360KV** (1:42) – wrists (2 per arm: flexion/abduction, actuators placed in the forearm with Bowden cables).  
  – **10× 3205 110KV** (1:42) – fingers (5 per arm, flexion).
...
- Two‑stage design; overall reduction ratios:  
  ...
  - 1:64 – for medium (5065) and basic (5010) in hip joints  
  - 1:42 – for low‑power (3205) and basic (5010) in shoulders, elbows and wrists  
- Rolling elements are standard bearing rollers (cheap and wear‑resistant).  
- Housings are cast in aluminium or printed in PETG CF/GF.
...
- **Battery:** 10S Li‑NMC (SK 3.7V 80.5Ah, pouch cells) – high energy density at moderate weight.  
...
- **Bus capacitors:** 10× 4700 µF / 63V, 15× 2200 µF / 35V – dampen regenerative spikes.  
- **Local filtering:** 0.1 µF ceramic capacitors on every SPI/I2C line and driver board to suppress PWM noise, and LC filters after the 5V and 24V buck converters.  
- **Wiring:** signal lines use shielded MKESh cable, shields grounded at a single point (the negative distribution block) to avoid ground loops. AWG30 in twisted pair for CAN.

All DC‑DC converters (1500W, 600W) are **oversized** and operate **without active cooling** – no additional fans for them here.
...
2. **Middle level (ESP32‑S3)**  
   Receives CAN from STM32F103C8T6 with information from: 13× BMI160, 4× HX711 ADCs, and an array of FSR402. Fuses IMU data for orientation estimation and forwards it to the laptop. Also serves as the **CAN gateway** for all motor controllers.

3. **Low level (joint controllers)**  
   – **ODrive3.6** (×3) – drive 6 high‑power motors (6384) via CAN.  
   – **Hoverboard mainboards** (×9) – each drives 2 motors (5065 or 5010). Total 18 channels, 17 actually used (7 medium + 10 basic). These boards have **UART only**, so we use **AT32F403ACGU7/STM32F103C8T6** as UART‑to‑CAN bridges: one MCU polls 2–3 hoverboard boards and relays data to the ESP32 over CAN.  
   ...
   – **STM32F103C8T6** (×10) – polls 13× BMI160, 4× HX711 ADCs, and the FSR402 array (through a CD74HC4067 multiplexer), and converts all analogue/I2C/SPI/UART signals to CAN, sending them to the ESP32‑S3 via SN65HVD230.

### Sensor Network

- **IMU nodes:** 13× BMI160 and 1× ICM‑20948 – connected to STM32F103C8T6 via SPI.
- **Tactile:** 10× FSR402 on fingers → analogue multiplexer CD74HC4067 → STM32F103C8T6 ADC.
- **Foot force:** 4× XNQJALYCY 100 kg load cells (2 per foot) → HX711 → STM32F103C8T6.
...
* [`weight_budget.en.md`](./docs/weight_budget.en.md) – Mass breakdown, centre‑of‑mass estimation, material choices (aluminium / PETG / PETG‑CF).
